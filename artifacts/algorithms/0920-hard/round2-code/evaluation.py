...
THRESHOLDS = {
    ...
    "entry_arc_m": 0.5,
    "identity_neighborhood_m": 0.75,
    "approach_improvement_m": 0.01,
    ...
    "avoidance_approach_m": 2.5,
    "avoidance_departure_m": 2.5,
    "avoidance_max_offset_m": 1.2,
}
...
class Evaluator:
    def __init__(self, scene: Scene):
        ...
        self.best_entry_distance = float(
            np.linalg.norm(self.previous - self.path.points[0])
        )
        self.last_approach_at = 0.0
        # Time to accelerate and reach the gate at the public speed limit, plus
        # the normal loss tolerance. Continued approach renews this allowance.
        self.acquisition_grace_s = (
            THRESHOLDS["offtrack_timeout_s"]
            + self.best_entry_distance / scene.vehicle.max_speed_mps
        ...
        self.avoidance_zones = self._avoidance_zones()
    ...
    def update(self, state: VehicleState, timestamp: float) -> dict:
        ...
        if entry_distance < self.best_entry_distance - t["approach_improvement_m"]:
            self.best_entry_distance = entry_distance
            self.last_approach_at = timestamp
        other_route = self._other_route(p)
        wrong_line = (
            other_route is not None
            and other_route["distance_m"] < t["switch_distance_m"]
            and other_route["distance_m"] + t["switch_margin_m"] < error
        )
        if (
            self.acquired_at is None
            and entry_distance <= t["acquire_distance_m"]
            and arc <= t["acquire_distance_m"]
        ...
        avoidance = (
            ...
            and any(
                ...
                for low, high, width in self.avoidance_zones
            )
        )
        if self.acquired_at is not None:
            if (ontrack or avoidance) and self.done_reason is None:
                ...
                self.offtrack_s += dt
        elif timestamp >= self.acquisition_grace_s:
            self.offtrack_s = max(0.0, timestamp - self.last_approach_at)

        # Initial placement may lie near another line. Allow time to approach
        # the actual start before confirming an unacquired wrong-line episode.
        identity_active = self.acquired_at is not None or (
            timestamp >= self.acquisition_grace_s
            and timestamp - self.last_approach_at >= t["switch_duration_s"]
        )
        self.switch_s = self.switch_s + dt if wrong_line and identity_active else 0
        if (
            self.switch_s + 1e-9 >= t["switch_duration_s"]
            ...
            self.switches.append(
                {
                    ...
                    "phase": (
                        "tracking" if self.acquired_at is not None else "acquisition"
                    ),
                    "reference_distance_m": error,
        ...
        if (
            self.offtrack_s + 1e-9 >= t["offtrack_timeout_s"]
            and self.done_reason is None
        ):
            self.done_reason = (
                "deviation" if self.acquired_at is not None else "acquisition_failed"
            )
        if (
            self.acquired_at is not None
            and ontrack
        ...
        identity = (
            ...
            if self.acquired_at is not None and ontrack
            else "unconfirmed"
        )
        self.last_evaluation = {
            ...
            "route_identity": "wrong_branch" if wrong_line else identity,
            "reference_arc_m": arc,
            "reference_window_m": [lower, min(self.path.total, upper)],
            "entry_distance_m": entry_distance,
            "acquisition_grace_s": self.acquisition_grace_s,
            "other_route": other_route,
            "wrong_route_duration_s": self.switch_s,
            "offtrack_duration_s": self.offtrack_s,
            "reason": self.done_reason,
            "collision_ids": collision_ids,
    ...
    def _avoidance_zones(self):
        """Bounded exemptions only where a solid actually blocks the ordered road.

        This private check applies equally to every policy. Other-road identity,
        collision, progress rate, start gate and finish gate remain mandatory.
        """
        zones = []
        padding = self.scene.vehicle.width_m / 2 + 0.08
        for index, polygon, _, _, _ in self.object_geometry:
            contour = polygon.astype(np.float32)
            distance = np.array(
                [
                    max(
                        0,
                        -cv2.pointPolygonTest(contour, tuple(map(float, point)), True),
                    )
                    for point in self.path.points
                ]
            )
            covered = np.flatnonzero(distance < padding)
            if not len(covered):
                continue
            # One physical prop can border several distinct route stretches.
            for group in np.split(covered, np.flatnonzero(np.diff(covered) > 1) + 1):
                obj = self.scene.objects[index]
                width = min(
                    THRESHOLDS["avoidance_max_offset_m"],
                    max(obj.width_m, obj.length_m) / 2 + padding + 0.45,
                )
                zones.append(
                    (
                        max(
                            0,
                            self.path.arc[group[0]]
                            - THRESHOLDS["avoidance_approach_m"],
                        ),
                        min(
                            self.path.total,
                            self.path.arc[group[-1]]
                            + THRESHOLDS["avoidance_departure_m"],
                        ),
                        width,
                    )
                )
    ...
    def _other_route(self, point: np.ndarray) -> dict | None:
        """Compare alternatives against the ordered reference, never a global target.

        A distant arc on the *same* polyline has a different route identity until
        the intervening route has been driven. The neighborhood exclusion keeps
        ordinary curved continuation out of the alternative-branch candidates.
        """
        candidates = []
        for index, line in enumerate(self.distractors):
            arc, distance, _ = line.project(point)
            candidates.append(
                {
                    "kind": "distractor",
                    "index": index,
        ...
        for low, high in intervals:
            if high <= low:
                continue
            arc, distance, _ = self.path.project(point, low, high)
            candidates.append(
                {
                    "kind": "target_nonlocal",
                    "index": None,
                    "arc_m": arc,
                    "distance_m": distance,
                }
...
def summarize(
    ...
    avoiding = [r for r in scored if r["evaluation"].get("avoidance_active")]
    track = [
        r
        for r in scored
        if r["evaluation"]["phase"] == "tracking"
        and not r["evaluation"].get("avoidance_active")
    ]
    ...
    metrics = {
        ...
        "avoidance_frames": len(avoiding) if evaluator else None,
        "avoidance_duration_s": len(avoiding) * dt if evaluator else None,
