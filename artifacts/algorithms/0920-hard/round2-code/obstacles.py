...
class ObstacleMemory:
    ...
    def observe(self, rgb, camera, hint, dt):
        ...
        mask = cv2.morphologyEx(
            chromatic.astype(np.uint8), cv2.MORPH_CLOSE, np.ones((7, 3), np.uint8)
        )
...
class DetourPlanner:
    ...
    def command_path(self, track, components, obstacles, dt):
        ...
        if self.path is not None:
            ...
            if self.blocking.center[0] < -0.3 and rejoined and route is not None:
                self.path = self.reference = None
                self.peek_attempted = False
        ...
        cloud = (
            np.concatenate([metric for _, metric in components])
            if components
            else np.empty((0, 2))
        )
        if len(cloud):
        ...
        best = None
        self.rejections = {
            key: 0 for key in ("visibility", "curvature", "obstacle", "other_route")
        }
        for side in (-1, 1):
            for offset in (0.45, 0.6, 0.75, 0.9):
                start = max(0.15, obstacle_arc - blocker.radius - 1.8)
                end = obstacle_arc + blocker.radius + 2.0
                if end > sample[-1] + 0.1:
                ...
                if (
                    len(cloud)
                    and np.min(np.linalg.norm(candidate[:, None] - cloud[None], axis=2))
                    < 0.18
                ):
    ...
    def peek_path(self, route, components, obstacles, blocker):
        ...
        candidates = []
        for side in (-1, 1):
            path = np.c_[x, side * 0.45 * (1 - np.cos(np.pi * x / length)) / 2]
            if any(
...
class AvoidanceTracker(TargetTracker):
    ...
    def observe(self, observation, dt):
        ...
        return super().observe(observation, dt)
